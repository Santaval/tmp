...
void VSocket::BuildSocket( char t, bool IPv6 ){
  ...
  if ( -1 == st ) {
    throw std::runtime_error("VSocket::BuildSocket, No valid socket type");
  }
  this->type = t;
  this->IPv6 = IPv6;
...
int VSocket::EstablishConnection( const char *host, const char *service ) {
  ...
  int status = getaddrinfo("::1", "8080", &hints, &results); 
 ...
 *
 *  Send data to another network point with connection 
 *
**/
int VSocket::sendMsg(const void* buffer, size_t size) {
  int st = -1;
  st = send(this->idSocket, buffer, size, 0);
  if (st == -1) {
    std::cerr << "SendMsg failed: " << strerror(errno) << std::endl;
    throw std::runtime_error( "VSocket::sendMsg" );
  }
  return st;
}
 ...
 *  Listen method
 *
 *  @param	int max length of pending connections queued
 *
 *  Listen for arriving connections in socket
 *
**/
int VSocket::Listen(int connections)  {
  int st = -1;
  st = listen(this->idSocket, connections);
  if (st == -1) {
