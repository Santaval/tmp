...
Thread::Finish ()
{   
    // set free the parent if assigned
    if (parentWaiting != nullptr) {
        scheduler->ReadyToRun(parentWaiting);
    }
    // remove this finishing thread from table
    threadTable->removeThread(id);
    // sete new level and update attributes
...
//----------------------------------------------------------------------
//  Thread::Join (added)
// 	Looks for thread in table and sets a child as that thread.
//  If thread is not finised, set current Thread as its parent
//----------------------------------------------------------------------

void
Thread::Join (int Tid){
    if (!threadTable->exists(Tid)) {
        printf("Thread does not exist %d\n", Tid);
        return;
    }
    Thread* child = threadTable->getThread(Tid);

    if (child == currentThread) {
        printf("Can't join to itself\n");
        return;
    }
    if (!child->finished) {
        child->parentWaiting = currentThread;
        currentThread->Sleep();
    }
